...
class Grader:
    def __init__(self, args=sys.argv):
        ...
        if 'p-' in assignment_name:
            num_points = 0
        self.addManualPart('style', maxPoints=num_points, extraCredit=True,
                           description='whether writeup is nicely typed, etc.')

    def addBasicPart(self, name, gradeFunc, maxPoints=1, maxSeconds=defaultMaxSeconds, extraCredit=False,
                     description=""):
    ...
    def addHiddenPart(self, name, gradeFunc, maxPoints=1, maxSeconds=defaultMaxSeconds, extraCredit=False,
                      description=""):
    ...
    def gradePart(self, part):
        print(
            '----- START PART %s%s: %s' % (part.name, ' (extra credit)' if part.extraCredit else '', part.description))
    ...
    def requireIsOneOf(self, trueAnswers, predAnswer):
        if predAnswer in trueAnswers:
            ...
            self.truncateString(trueAnswers), self.truncateString(predAnswer)))
    ...
    def requireIsEqual(self, trueAnswer, predAnswer, tolerance=TOLERANCE):
        if isEqual(trueAnswer, predAnswer, tolerance):
            ...
            self.truncateString(str(trueAnswer)), self.truncateString(str(predAnswer))))
